  ...
  group('RenderTag', () {
    group('sync evaluation', () {
      ...
      test('throws exception for non-existent template', () async {
        ...
        final throwingRoot = FileSystemRoot(
          '/templates',
    ...
    group('scope isolation tests - sync', () {
      test(
        'variables assigned inside render do not leak to parent scope',
        () async {
        ...
        ''',
            (document) {
              ...
              expect(
                result,
                'Before: original original After: original Secret:',
              );
      ...
      test(
        'global variables must be explicitly passed to be accessible',
        () async {
          evaluator.context.setVariable('global_var', 'global_value');
      ...
      test(
        'modifications to passed variables do not affect parent scope',
        () async {
          evaluator.context.setVariable('passed_var', 'original');
        ...
        ''',
            (document) {
              ...
              expect(
                result,
                'Before: original Modified: modified After: original',
              );
            },
          );
        },
      ...
      test(
        'passed variables override parent scope variables with same name',
        () async {
...
''');
        ...
        ''',
            (document) {
              ...
              expect(
                result,
                contains('Name: passed_name'),
              ); // Inside render uses passed value
              expect(
                result,
                contains('Category: passed_category'),
              ); // Inside render uses passed value
              expect(
                result,
                contains('Parent var:'),
              ); // Empty because not passed
              expect(
                result,
                contains('After render name: parent_value'),
              ); // Parent unchanged
              expect(
                result,
                contains('After render category: parent_category'),
              ); // Parent unchanged

              // Verify parent scope variables are unchanged
              expect(evaluator.context.getVariable('name'), 'parent_value');
              expect(
                evaluator.context.getVariable('category'),
                'parent_category',
              );
            },
          );
        ...
        ''',
          (document) {
            ...
            expect(
              result,
              contains('L2 l1_only:'),
            ); // Empty - from parent render
            expect(result, contains('L1 after: level1level1_var'));
            expect(
              result,
              contains('Root after: rootroot_shared'),
            ); // No leakage
    ...
    group('async evaluation', () {
      ...
      test('throws exception for non-existent template', () async {
        ...
        await testParser('{% render "non_existent.liquid" %}', (
          ...
          expect(
            () => evaluator.evaluateNodesAsync(document.children),
            throwsException,
          );
        });
      });
    ...
    group('scope isolation tests - async', () {
      ...
      test(
        ...
        () async {
          ...
          await testParser('{% render "try_global.liquid" %}', (
            document,
          ) async {
            await evaluator.evaluateNodesAsync(document.children);
