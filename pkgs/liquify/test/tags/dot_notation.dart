  ...
  group('Dot notation filters', () {
    setUp(() {
      ...
      FilterRegistry.register(
        'size',
        (value, args, namedArgs) => value.length,
        dotNotation: true,
      );
      FilterRegistry.register(
        'first',
        (value, args, namedArgs) => value.first,
        dotNotation: true,
      );
      FilterRegistry.register(
        'last',
        (value, args, namedArgs) => value.last,
        dotNotation: true,
      );
    });
    ...
    test('should apply size filter using dot notation', () {
      ...
      final node = MemberAccess(Identifier('site'), [
        Identifier('pages'),
        Identifier('size'),
      ]);
    ...
    test('should apply first filter using dot notation', () {
      ...
      final evaluator = Evaluator(context);
      final node = MemberAccess(Identifier('collection'), [
        Identifier('products'),
        Identifier('first'),
      ]);
    ...
    test(
      ...
      () {
        ...
        FilterRegistry.register(
          'custom',
          (value, args, namedArgs) => 'custom filter applied',
          dotNotation: true,
        );

        final context = Environment({'data': 'test'});
        final evaluator = Evaluator(context);
        final node = MemberAccess(Identifier('data'), [Identifier('custom')]);
