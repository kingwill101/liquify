...
void main() {
  ...
  group('Dot notation filters', () {
    group("dot notation filters sync", () {
      test('first, last and size', () {
        ...
        expect(
          evaluator.evaluate(
            MemberAccess(Identifier('company'), [
  ...
  group('Evaluator', () {
    ...
    test('evaluates literals asynchronously', () async {
      ...
      expect(
        await evaluator.evaluateAsync(Literal(true, LiteralType.boolean)),
        true,
      );
      expect(
    ...
    test('evaluates empty literals on strings asynchronously', () async {
      ...
      expect(
        (await evaluator.evaluateAsync(
          Literal('not empty', LiteralType.string),
        )).isEmpty,
        false,
      );
    });
    ...
    test('evaluates empty literals on arrays', () {
      ...
      expect(
        evaluator.evaluate(Literal([1, 2, 3, 4, 5], LiteralType.array)).isEmpty,
        false,
      );
    });
    ...
    test('evaluates empty literals on arrays asynchronously', () async {
      ...
      expect(
        (await evaluator.evaluateAsync(Literal([], LiteralType.array))).isEmpty,
        true,
      );
      expect(
        (await evaluator.evaluateAsync(
          Literal([1, 2, 3, 4, 5], LiteralType.array),
        )).isEmpty,
        false,
      );
    });
    ...
    test('evaluates nested assignment with filtered expression', () {
      ...
      final nestedAssignment = Assignment(
        ...
        FilteredExpression(
          Assignment(
            Identifier('start_page'),
            MemberAccess(Identifier('paginator'), [Identifier('current_page')]),
          ),
          [
    ...
    test(
      'evaluates nested assignment with filtered expression asynchronously',
      () async {
        // Set up context data
        evaluator.context.setVariable('paginator', {'current_page': 5});
        ...
        final nestedAssignment = Assignment(
          ...
          FilteredExpression(
            Assignment(
              Identifier('start_page'),
              MemberAccess(Identifier('paginator'), [
    ...
    test(
      'evaluates assignment with filtered expression asynchronously',
      () async {
        ...
        expect(
          await evaluator.evaluateAsync(Identifier('uppercased_name')),
          'John',
        );
      },
    );
    ...
    test('evaluates assignment with identifier value', () {
      ...
      evaluator.evaluate(
        Assignment(Identifier('y'), Literal(100, LiteralType.number)),
      );

      // Now, create an assignment that uses this identifier
      final assignment = Assignment(Identifier('z'), Identifier('y'));
    ...
    test(
      'evaluates assignment with complex expression asynchronously',
      () async {
        final complexAssignment = Assignment(
          Identifier('result'),
          BinaryOperation(
    ...
    test('evaluates unary operations', () {
      final notOperation = UnaryOperation(
        'not',
    ...
    test('evaluates grouped expressions', () {
      final grouped = GroupedExpression(
        BinaryOperation(
          Literal(2, LiteralType.number),
          '+',
          Literal(3, LiteralType.number),
    ...
    test('evaluates member access', () {
      ...
      final memberAccess = MemberAccess(Identifier('user'), [
        Identifier('name'),
      ]);
  ...
  group('Context data evaluation', () {
    test('evaluates existing context data', () {
      evaluator.context.setVariable('user', {
        'name': 'Alice',
        'age': 30,
        'last-name': 'Doe',
      });
    ...
    test(
      'returns null for missing top-level context data asynchronously',
      () async {
    ...
    test(
      'returns null for missing nested context data asynchronously',
      () async {
        evaluator.context.setVariable('user', {'name': 'Bob'});
        expect(
          await evaluator.evaluateAsync(
    ...
    test('evaluate array int access', () {
      ...
      expect(
        evaluator.evaluate(
          ArrayAccess(Identifier('numbers'), Literal(0, LiteralType.number)),
        ),
        1,
      );
    });
    ...
    test('evaluate array string access', () {
      evaluator.context.setVariable('name', {'first': 'John'});
      expect(
        evaluator.evaluate(
          ArrayAccess(Identifier('name'), Literal('first', LiteralType.string)),
        ),
        equals('John'),
      );
    });
