  ...
  group('Environment-Scoped Registry Tests', () {
    ...
    group('Environment-Scoped Filters', () {
      ...
      test('should isolate filters between different environments', () {
        ...
        env1.registerLocalFilter(
          'env1_filter',
          (value, args, namedArgs) => 'env1-$value',
        );
        env2.registerLocalFilter(
          'env2_filter',
          (value, args, namedArgs) => 'env2-$value',
        );
      ...
      test(
        'should fallback to global registry when local filter not found',
        () {
          final env = Environment();
          ...
          FilterRegistry.register(
            'global_filter',
            (value, args, namedArgs) => 'global-$value',
          );
      ...
      test('should prioritize local filters over global ones', () {
        ...
        env.registerLocalFilter(
          'shared_name',
          (value, args, namedArgs) => 'local-$value',
        );
    ...
    group('Environment-Scoped Tags', () {
      ...
      test('should prioritize local tags over global ones', () {
        ...
        env.registerLocalTag(
          'shared_tag',
          (content, filters) => LocalTestTag(content, filters),
        );
    ...
    group('Integration Tests - Actual Rendering', () {
      ...
      test(
        'should verify local filter functionality without full rendering',
        () {
          final env = Environment();
      ...
      test(
        'should verify template rendering works with built-in tags',
        () async {
        ...
        () async {
          ...
          final template = Template.parse(
            ...
            environmentSetup: (env) {
              ...
              env.registerLocalTag(
                'custom_tag',
                (content, filters) => DemoTag(content, filters),
              );
            },
          );
      ...
      test(
        'should demonstrate template with custom environment instance',
        () async {
          ...
          customEnv.registerLocalFilter(
            'safe',
            (value, args, namedArgs) => 'SAFE:$value',
          );
      ...
      test(
        'should demonstrate environment isolation between templates',
        () async {
          ...
          final template1 = Template.parse(
            ...
            environmentSetup: (env) {
              env.registerLocalFilter(
                'format',
                (value, args, namedArgs) => 'TEMPLATE1:$value',
              );
            },
          );
          ...
          final template2 = Template.parse(
            ...
            environmentSetup: (env) {
              env.registerLocalFilter(
                'format',
                (value, args, namedArgs) => 'TEMPLATE2:$value',
              );
            },
          );
      ...
      test(
        'should allow access to environment after template creation',
        () async {
          final template = Template.parse(
            '{{ value | dynamic_filter }}',
            data: {'value': 'test'},
          );
          ...
          template.environment.registerLocalFilter(
            'dynamic_filter',
            (value, args, namedArgs) => 'DYNAMIC:$value',
          );
      ...
      test('should verify strict mode blocks global filters', () {
        ...
        FilterRegistry.register(
          'dangerous',
          (value, args, namedArgs) => 'DANGER: $value',
        );
    ...
    group('Environment Cloning', () {
      test('should clone local filters and tags with environment', () {
        ...
        original.registerLocalFilter(
          'test_filter',
          (value, args, namedArgs) => 'filtered-$value',
        );
    ...
    group('Security and Sandboxing', () {
      test('should create secure environment with limited filters', () {
        ...
        secureEnv.registerLocalFilter(
          'escape',
          (value, args, namedArgs) => 'escaped-$value',
        );
        secureEnv.registerLocalFilter(
          'truncate',
      ...
      test(
        'should create Environment with strict mode using named constructor',
        () {
          final strictEnv = Environment.withStrictMode();
