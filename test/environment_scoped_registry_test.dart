  ...
  group('Environment-Scoped Registry Tests', () {
    ...
    group('Environment-Scoped Filters', () {
      ...
      test('should isolate filters between different environments', () {
        ...
        env1.registerLocalFilter(
            'env1_filter', (value, args, namedArgs) => 'env1-$value');
        env2.registerLocalFilter(
            'env2_filter', (value, args, namedArgs) => 'env2-$value');
      ...
      test('should fallback to global registry when local filter not found',
        ...
        FilterRegistry.register(
            'global_filter', (value, args, namedArgs) => 'global-$value');
      ...
      test('should prioritize local filters over global ones', () {
        ...
        FilterRegistry.register(
            'shared_name', (value, args, namedArgs) => 'global-$value');

        // Register local filter with same name
        env.registerLocalFilter(
            'shared_name', (value, args, namedArgs) => 'local-$value');
      ...
      test('should list all available filters (local + global)', () {
        ...
        expect(availableFilters,
            containsAll(['global1', 'global2', 'local1', 'local2']));
      });
    });
    ...
    group('Environment-Scoped Tags', () {
      test('should register and retrieve environment-scoped tags', () {
        ...
        env.registerLocalTag('custom_tag',
            (content, filters) => CustomTestTag(content, filters));
      ...
      test('should isolate tags between different environments', () {
        ...
        env1.registerLocalTag(
            'env1_tag', (content, filters) => CustomTestTag(content, filters));
        env2.registerLocalTag(
            'env2_tag', (content, filters) => CustomTestTag(content, filters));
      ...
      test('should fallback to global registry when local tag not found', () {
        ...
        TagRegistry.register('global_tag',
            (content, filters) => CustomTestTag(content, filters));
      ...
      test('should prioritize local tags over global ones', () {
        ...
        // Register global tag
        TagRegistry.register('shared_tag',
            (content, filters) => GlobalTestTag(content, filters));

        // Register local tag with same name
        env.registerLocalTag(
            'shared_tag', (content, filters) => LocalTestTag(content, filters));
      ...
      test('should list all available tags (local + global)', () {
        ...
        TagRegistry.register('global_tag1',
            (content, filters) => CustomTestTag(content, filters));
        TagRegistry.register('global_tag2',
            (content, filters) => CustomTestTag(content, filters));

        // Register local tags
        env.registerLocalTag('local_tag1',
            (content, filters) => CustomTestTag(content, filters));
        env.registerLocalTag('local_tag2',
            (content, filters) => CustomTestTag(content, filters));
        ...
        expect(
            availableTags,
            containsAll(
                ['global_tag1', 'global_tag2', 'local_tag1', 'local_tag2']));
      });
    });

    group('Integration Tests - Actual Rendering', () {
      test(
          'should demonstrate environment-scoped filters work with global fallback',
          () async {
        ...
        // Register a global filter
        FilterRegistry.register(
            'test_filter', (value, args, namedArgs) => 'GLOBAL: $value');

        // Register a local filter with the same name
        env.registerLocalFilter(
            'test_filter', (value, args, namedArgs) => 'LOCAL: $value');
      ...
      test('should verify tag creator functions work', () {
        ...
        env.registerLocalTag(
            'greet', (content, filters) => GreetTag(content, filters));
      ...
      test('should verify template rendering works with built-in tags',
          () async {
      ...
      test(
          'should demonstrate actual template rendering with environment setup callback',
          () async {
        ...
        final template = Template.parse(
          ...
          environmentSetup: (env) {
            ...
            env.registerLocalTag(
                'custom_tag', (content, filters) => DemoTag(content, filters));
          },
        );

        final result = await template.renderAsync();
        expect(result.trim(),
            equals('Hello World! [DEMO: ] Result: ***SUCCESS***'));
      });

      test('should demonstrate template with custom environment instance',
          () async {
        // Create a custom environment with pre-configured filters/tags
        final customEnv =
            Environment.withStrictMode(); // Strict mode for security
        customEnv.registerLocalFilter(
            'safe', (value, args, namedArgs) => 'SAFE:$value');
        customEnv.registerLocalTag(
            'secure_tag', (content, filters) => GreetTag(content, filters));
      ...
      test('should demonstrate environment isolation between templates',
          () async {
        ...
        final template1 = Template.parse(
          ...
          environmentSetup: (env) {
            env.registerLocalFilter(
                'format', (value, args, namedArgs) => 'TEMPLATE1:$value');
          },
        );
        ...
        final template2 = Template.parse(
          ...
          environmentSetup: (env) {
            env.registerLocalFilter(
                'format', (value, args, namedArgs) => 'TEMPLATE2:$value');
          },
        );
      ...
      test('should allow access to environment after template creation',
          () async {
        final template = Template.parse('{{ value | dynamic_filter }}',
            data: {'value': 'test'});

        // Register filter after template creation
        template.environment.registerLocalFilter(
            'dynamic_filter', (value, args, namedArgs) => 'DYNAMIC:$value');
      ...
      test('should verify filter isolation between environments', () {
        ...
        // Register different filters in each environment
        env1.registerLocalFilter(
            'format', (value, args, namedArgs) => 'ENV1: $value');
        env2.registerLocalFilter(
            'format', (value, args, namedArgs) => 'ENV2: $value');
      ...
      test('should verify strict mode blocks global filters', () {
        ...
        strictEnv.registerLocalFilter(
            'safe', (value, args, namedArgs) => 'SAFE: $value');

        // Register a global filter
        FilterRegistry.register(
            'dangerous', (value, args, namedArgs) => 'DANGER: $value');
    ...
    group('Environment Cloning', () {
      test('should clone local filters and tags with environment', () {
        ...
        original.registerLocalFilter(
            'test_filter', (value, args, namedArgs) => 'filtered-$value');
        original.registerLocalTag(
            'test_tag', (content, filters) => CustomTestTag(content, filters));
        ...
        cloned.registerLocalFilter(
            'clone_only', (value, args, namedArgs) => value);
    ...
    group('Security and Sandboxing', () {
      test('should create secure environment with limited filters', () {
        ...
        secureEnv.registerLocalFilter(
            'escape', (value, args, namedArgs) => 'escaped-$value');
        secureEnv.registerLocalFilter('truncate',
            (value, args, namedArgs) => value.toString().substring(0, 5));

        // Register dangerous global filter
        FilterRegistry.register(
            'dangerous_filter', (value, args, namedArgs) => 'DANGER');
      ...
      test('should support strict mode that blocks global registry access', () {
        final strictEnv =
            Environment.withStrictMode(); // Create with strict mode enabled

        // Register local filter
        strictEnv.registerLocalFilter(
            'safe_filter', (value, args, namedArgs) => 'safe-$value');

        // Register global filter
        FilterRegistry.register(
            'global_filter', (value, args, namedArgs) => 'global');
